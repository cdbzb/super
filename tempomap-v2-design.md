...
- **Application** pushes data through a map: scalar `at`/`invAt`, and span mapping
  (`warp(deltas, from:)` / `unwarp`) written ONCE generically as differences of
  cumulative `at` — deleting the per-class `mapBeats`/`warp`/`warpDurs` copies.
...
5. **Bridge, not delegation** (reframed 2026-07-29). TempoMap's warts ARE its
   ...
   b. `array.warpTo(aMonoMap)` → `warp` in the plusArray dispatch — the
